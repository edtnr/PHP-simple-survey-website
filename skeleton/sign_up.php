...
elseif (isset($_POST['username']))
{
    ...
    $phone_val = validateString($phone, 1, 64);
    $dob_val = validateString($DOB, 1, 64);
    // concatenate all the validation results together ($errors will only be empty if ALL the data is valid):
    $errors = $username_val . $password_val . $email_val . $phone_val . $dob_val ;
...
<form action="sign_up.php" method="post">
  ...
  <br>  
  Phone Number: <input type="tel" name="phone" maxlength="64" value="$phone" required> $phone_val
  <br>
   Date of Birth: <input type="date" name="date" maxlength="64" value="$DOB" required> $dob_val
  <br>
